...
class TenderRts(
    ...
    val applGuarantee: String,
    val currency: String,
    val contrGuarantee: String,
    val nmck: String
) {
    companion object O {
        const val typeFz = 17
    ...
    fun parsing() {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            var IdEtp = 0
            val etpName = "РТС-тендер"
            val etpUrl = "http://corp.rts-tender.ru"
            try {
            ...
            insertLot.close()
            var idCustomer = 0
            val fullnameCus = html.selectFirst("td:containsOwn(Заказчик) + td a span")?.ownText()?.trim()
                ?: ""
            if (fullnameCus != "") {
                val stmto =
            ...
            val purchObj: Elements =
                html.select("div:containsOwn(Информация об объектах) + div table.table-carts tbody tr")
            purchObj.forEach { po ->
                ...
                val insertPurObj =
                    con.prepareStatement("INSERT INTO ${Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, quantity_value = ?, okei = ?, customer_quantity_value = ?, okpd2_code = ?, okpd2_group_code = ?, okpd2_group_level1_code = ?, price = ?")
            ...
            if (delivPlace != "") {
                val insertCusRec =
                    con.prepareStatement("INSERT INTO ${Prefix}customer_requirement SET id_lot = ?, id_customer = ?, delivery_place = ?, application_guarantee_amount = ?, max_price = ?, contract_guarantee_amount = ?")
