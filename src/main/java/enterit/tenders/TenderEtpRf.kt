...
class TenderEtpRf(val status: String, val entNum: String, var purNum: String, val purObj: String, val nmck: String, val placingWay: String, val datePub: Date, val dateEnd: Date, val url: String) {
    ...
    fun parsing() {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            if (innOrg != "") {
                val stmto = con.prepareStatement("SELECT id_organizer FROM ${Prefix}organizer WHERE inn = ? AND kpp = ?")
                ...
                if (rso.next()) {
                    ...
                    val contactPerson = html.selectFirst("td:containsOwn(Контактное лицо) ~ td")?.ownText()?.trim()
                            ?: ""
                    val stmtins = con.prepareStatement("INSERT INTO ${Prefix}organizer SET full_name = ?, inn = ?, kpp = ?, post_address = ?, contact_person = ?, contact_email = ?, contact_phone = ?, contact_fax = ?", Statement.RETURN_GENERATED_KEYS)
            ...
            if (purObj.count() > 0) {
                purObj.forEach { po ->
                    ...
                    val insertPurObj = con.prepareStatement("INSERT INTO ${Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, quantity_value = ?, okei = ?, customer_quantity_value = ?, okpd2_code = ?, okpd2_group_code = ?, okpd2_group_level1_code = ?")
            ...
            val delivPlace = html.selectFirst("td:containsOwn(Место поставки, выполнения работ, оказания услуг) ~ td")?.ownText()?.trim { it <= ' ' }
                    ?: ""
            val delivTerm = html.selectFirst("td:containsOwn(Дополнительные комментарии) ~ td")?.ownText()?.trim { it <= ' ' }
                    ?: ""
            var applGAmount = html.selectFirst("td:containsOwn(Размер обеспечения(резервирования оплаты) заявки на участие, в рублях) ~ td")?.ownText()?.trim { it <= ' ' }
                    ?: ""
            applGAmount = applGAmount.replace("руб.", "")
            val applGuaranteeAmount = returnPriceEtpRf(applGAmount)
            if (delivPlace != "") {
                val insertCusRec = con.prepareStatement("INSERT INTO ${Prefix}customer_requirement SET id_lot = ?, id_customer = ?, delivery_term = ?, delivery_place = ?, application_guarantee_amount = ?, max_price = ?")
            ...
            val restr = html.selectFirst("td:containsOwn(Требование к отсутствию участника в реестре недобросовестных поставщиков) ~ td")?.ownText()?.trim { it <= ' ' }
                    ?: ""
            if (restr == "Да") {
            ...
            val msp = html.selectFirst("td:containsOwn(Торги для субъектов малого и среднего предпринимательства) ~ td")?.ownText()?.trim { it <= ' ' }
                    ?: ""
            if (msp == "Да") {
