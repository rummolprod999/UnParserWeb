...
public class TenderGpnAnn(val status: String, val url: String) {
    ...
    fun parsing() {
        ...
        val startDateT =
            html.selectFirst("div:contains(Начало приема заинтересованности:) + div")?.ownText()?.replace(",", "")
                ?.trim { it <= ' ' }
                ?: ""
        val endDateT =
            html.selectFirst("div:contains(Окончание приема заинтересованности:) + div")?.ownText()?.replace(",", "")
                ?.trim { it <= ' ' }
                ?: ""
        val startDate = getDateFromFormat(startDateT, formatterOnlyDate)
        val endDate = getDateFromFormat(endDateT, formatterOnlyDate)
        if (startDate == Date(0L)) {
            logger("Empty start date in $url")
            return
        }
        val purNum = html.selectFirst("div.info-number")?.ownText()?.replace("№", "")?.trim { it <= ' ' } ?: ""
        if (purNum == "") {
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            ...
            if (fullnameOrg != "") {
                ...
                if (rso.next()) {
                    ...
                    stmto.close()
                    val phone =
                        html.selectFirst("div:contains(Контактная информация:) + div > div:contains(Тел:)")?.text()
                            ?.trim { it <= ' ' }
                            ?: ""
                    val email =
                        html.selectFirst("div:contains(Контактная информация:) + div > div:contains(E-mail:)")?.text()
                            ?.trim { it <= ' ' }
                            ?: ""
                    val stmtins = con.prepareStatement(
                        "INSERT INTO ${Prefix}organizer SET full_name = ?, contact_email = ?, contact_phone = ?",
            ...
            insertTender.setInt(6, TenderGpn.typeFz)
            ...
            val documents: Elements = html.select("a.doc-title")
            ...
            insertLot.close()
            val insertPurObj =
                con.prepareStatement("INSERT INTO ${Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, quantity_value = ?, customer_quantity_value = ?")
            ...
            insertPurObj.close()
            val delivPlace = html.selectFirst("div:contains(Место поставки товаров) + div")?.text()
                ?.trim { it <= ' ' }
                ?: ""
            val delivTerm = html.selectFirst("div:contains(Описание:) + div")?.text()
                ?.trim { it <= ' ' }
                ?: ""
            if (delivPlace != "") {
                val insertCusRec =
            ...
            try {
                addVNum(con, purNum, TenderGpn.typeFz)
            } catch (e: Exception) {
