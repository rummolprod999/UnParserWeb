...
data class TenderPolNew(
    ...
    fun parsing() {
        ...
        val startDateT =
            html.selectFirst("table.decorated_table tbody tr td:eq(0) p.date")?.text()?.trim { it <= ' ' } ?: ""
        var startDate = getDateFromFormat(startDateT.replaceDatePolis(), formatterPolis)
        val endDateT =
            html.selectFirst("table.decorated_table tbody tr td:eq(4) p.date")?.text()?.trim { it <= ' ' } ?: ""
        var endDate = getDateFromFormat(endDateT.replaceDatePolis(), formatterPolis)
        if (startDate == Date(0L) && endDate == Date(0L)) {
            val detail = html.selectFirst("div.purchase-single-info")?.ownText()?.trim { it <= ' ' } ?: ""
            val (startDateNew, endDateNew, status) = this.GetDates(detail)
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            val stmt0 =
                con.prepareStatement("SELECT id_tender FROM ${Prefix}tender WHERE purchase_number = ? AND date_version = ? AND type_fz = ? AND notice_version = ? AND end_date = ? AND href = ?")
            ...
            if (orgName != "") {
                ...
                if (rso.next()) {
                    ...
                    rso.close()
                    stmto.close()
                    val contactP = html.selectFirst("div.contact__info > p:eq(0)")?.text()?.trim { it <= ' ' }
                        ?: ""
                    val phone = html.selectFirst("div.contact__info > p:eq(1)")?.text()?.trim { it <= ' ' }
                        ?: ""
                    val stmtins = con.prepareStatement(
                        "INSERT INTO ${Prefix}organizer SET full_name = ?, contact_phone = ?, contact_person = ?",
            ...
            val etpUrl = "https://tenders.polyus.com/"
            try {
            ...
            val insertTender = con.prepareStatement(
                "INSERT INTO ${Prefix}tender SET id_region = 0, id_xml = ?, purchase_number = ?, doc_publish_date = ?, href = ?, purchase_object_info = ?, type_fz = ?, id_organizer = ?, id_placing_way = ?, id_etp = ?, end_date = ?, cancel = ?, date_version = ?, num_version = ?, notice_version = ?, xml = ?, print_form = ?",
                Statement.RETURN_GENERATED_KEYS
            )
