...
class TenderStg(
    ...
    override fun parsing() {
        ...
        var pubDateT =
            page.getFirstByXPath<HtmlSpan>("//td[contains(preceding-sibling::td, 'Дата публикации')]/span")?.textContent?.trim { it <= ' ' }
                ?: ""
        ...
        var endDateT =
            page.getFirstByXPath<HtmlSpan>("//td[contains(preceding-sibling::td, 'Дата окончания подачи заявок')]/span")?.textContent?.trim { it <= ' ' }
                ?: ""
        ...
        DriverManager.getConnection(UrlConnect, UserDb, PassDb).use(fun(con: Connection) {
            val stmt0 =
                con.prepareStatement("SELECT id_tender FROM ${Prefix}tender WHERE purchase_number = ? AND doc_publish_date = ? AND type_fz = ? AND notice_version = ?")
            ...
            if (orgFullName != "") {
                ...
                if (rso.next()) {
                    ...
                    stmto.close()
                    val email =
                        page.getFirstByXPath<HtmlSpan>("//div[contains(preceding-sibling::div, 'Ответственный за процедуру')]//td[contains(preceding-sibling::td, 'Email')]/span")?.textContent?.trim { it <= ' ' }
                            ?: ""
                    val phone =
                        page.getFirstByXPath<HtmlSpan>("//div[contains(preceding-sibling::div, 'Ответственный за процедуру')]//td[contains(preceding-sibling::td, 'Телефон')]/span")?.textContent?.trim { it <= ' ' }
                            ?: ""
                    val contactPerson =
                        page.getFirstByXPath<HtmlSpan>("//div[contains(preceding-sibling::div, 'Ответственный за процедуру')]//td[contains(preceding-sibling::td, 'ФИО')]/span")?.textContent?.trim { it <= ' ' }
                            ?: ""
            ...
            var idTender = 0
            var scoringDateT =
                page.getFirstByXPath<HtmlSpan>("//td[contains(preceding-sibling::td, 'Дата рассмотрения заявки')]/span")?.textContent?.trim { it <= ' ' }
                    ?: ""
            ...
            val lots =
                page.getByXPath<HtmlDivision>("//div[contains(@class, 'tradeLotInfo') and contains(@class, 'labelminwidth')]")
            lots.forEach {
    ...
    private fun parsingLots(it: HtmlDivision, con: Connection, idTender: Int, page: HtmlPage) {
        ...
        val currency =
            it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Тип валюты')]/span")?.textContent?.trim { it <= ' ' }
                ?: ""
        val nmckT =
            it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Начальная/максимальная цена')]/span")?.textContent?.trim { it <= ' ' }
                ?: ""
        ...
        var idCustomer = 0
        val fullnameOrg =
            page.getFirstByXPath<HtmlSpan>("//td[contains(preceding-sibling::td, 'Заказчик')]/span")?.textContent?.trim { it <= ' ' }
                ?: ""
        if (fullnameOrg != "") {
        ...
        val positions =
            it.getByXPath<HtmlDivision>(".//div[div[contains(@class, 'box')]/div[@class = 'portlet-title']]")
        if (!positions.isEmpty()) {
            positions.forEach { pr ->
                val purName =
                    pr.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Наименование')]/span")?.textContent?.trim { it <= ' ' }
                        ?: ""
                var quantity =
                    pr.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Количество')]/span")?.textContent?.trim { it <= ' ' }
                        ?: ""
                quantity = quantity.extractPrice()
                val okei =
                    pr.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Ед. измерения')]/span")?.textContent?.trim { it <= ' ' }
                        ?: ""
                val okpd2T =
                    pr.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Код ОКПД 2')]/span")?.textContent?.trim { it <= ' ' }
                        ?: ""
                ...
                val insertPurObj =
                    con.prepareStatement("INSERT INTO ${Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, quantity_value = ?, okei = ?, customer_quantity_value = ?, okpd2_code = ?, okpd2_group_code = ?, okpd2_group_level1_code = ?, okpd_name = ?")
            ...
            val okpd2T =
                it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Код ОКПД 2')]/span")?.textContent?.trim { it <= ' ' }
                    ?: ""
            val okpd2Code = okpd2T.regExpTest("""^([\d\.]+)""")
            val (okpd2GroupCode, okpd2GroupLevel1Code) = getOkpd(okpd2Code)
            val okpd2Name = okpd2T.regExpTest("""^[\d\.]+ - (.+)${'$'}""")
            val quantity =
                it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Количество') and not(contains(preceding-sibling::td, 'блокировка'))]/span")?.textContent?.trim { it <= ' ' }
                    ?: ""
            val okei =
                it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Ед. измерения')]/span")?.textContent?.trim { it <= ' ' }
                    ?: ""
            val purName =
                it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Наименование')]/span")?.textContent?.trim { it <= ' ' }
                    ?: ""
            val insertPurObj =
                con.prepareStatement("INSERT INTO ${Prefix}purchase_object SET id_lot = ?, id_customer = ?, name = ?, quantity_value = ?, okei = ?, customer_quantity_value = ?, okpd2_code = ?, okpd2_group_code = ?, okpd2_group_level1_code = ?, okpd_name = ?, sum = ?")
            insertPurObj.setInt(1, idLot)
            insertPurObj.setInt(2, idCustomer)
            insertPurObj.setString(3, purName)
            insertPurObj.setString(4, quantity)
        ...
        val delivPlace =
            it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Место доставки поставляемых')]/span")?.textContent?.trim { it <= ' ' }
                ?: ""
        if (delivPlace != "") {
            val insertCusRec =
                con.prepareStatement("INSERT INTO ${Prefix}customer_requirement SET id_lot = ?, id_customer = ?, delivery_place = ?, max_price = ?")
        ...
        val rec =
            it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Особенности участия субъектов')]/span")?.textContent?.trim { it <= ' ' }
                ?: ""
        if (rec != "") {
        ...
        val pref =
            it.getFirstByXPath<HtmlSpan>(".//td[contains(preceding-sibling::td, 'Преференции для контрагентов')]/span")?.textContent?.trim { it <= ' ' }
                ?: ""
        if (pref != "") {
